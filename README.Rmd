...
A number of different earth sciences datasets are needed to initialize
a SWAT+ model:
...
watershed area, then downloads and processes the required datasets. It produces
a set of output files ready for QSWAT+, and retains copies of the source datasets
for later use.


## History

rswat_uyr is largely based on code in the URYW_data repository, now much simplified
so that it can be maintained as a self-contained R package. 

Our watershed of interest is the Upper Yellowstone, so testing and examples in rswat.uyr
focus on areas upstream of Carter's Bridge, Montana. This includes Paradise Valley and most
of Yellowstone National Park.

rswat.uyr is connected to rswat in that it serves as a source of example data.
However rswat is not required. The rswat.uyr workflow prepares inputs to QSWAT+,
whereas rswat manages the SWAT+ model config files that are created by QSWAT+.


## Motivation

The purpose of rswat.uyr is to automate and document ETL for the datasets required by
