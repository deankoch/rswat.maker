...
This article explains how to use `rswat.maker` to create a series of SWAT+ projects, using the [Lamar River](https://www.nps.gov/articles/000/lamar-river.htm) in Yellowstone National Park as an example. After introducing the big picture here, we walk users through each step in detail.

[SWAT+](https://swat.tamu.edu/software/plus/) is a process-based watershed simulator [@swatplusbieger2017introduction] familiar to many in the hydrology and water management communities for its scope, ease of access, and long history of active development. A graphical user interface (GUI) called [QSWAT+](https://swat.tamu.edu/software/qswat/) is provided for model constrcution. However, like most (distributed) hydrodynamic models SWAT+ is very data-hungry, and its reliance on complex workflows in GUIs create problems for reproducibility and collaboration [@awchawanda2020user; @hydromeilander2023].
...
```{r overview-catch-plot, cache=TRUE, echo=FALSE, warning=FALSE, fig.align='center', fig.show='hold', out.width='33%', fig.dim=c(4, 6), fig.cap='Three models of the Lamar River: from NHDPlus (left), rswat.maker (middle), and QSWAT+ (right).'}
...
```{r flowchart, echo=FALSE, fig.align='center', out.width='75%', fig.cap='A flow chart for `rswat.maker` splits the whole process into four phases'}
knitr::include_graphics('D:/rswat.maker/vignettes/articles/lamar_flowchart/flow_chart_rswat.png')
```

Run a single step by setting `what` to its name (see `?run_maker` for the list of step names). Those not interested in SWAT+ can also set the keyword `what='data'`, which runs only Phase "A" in Figure \@ref(fig:flowchart).
...
```{r qswat-plot, cache=TRUE, fig.show='hold', out.width='50%', fig.dim=c(6, 8), fig.align='center', fig.cap='Two models of Soda Butte Creek subcatchment geometries: from NHDPlus (left) and QSWAT+ (right)'}
...
Subcatchments give users a more fine-grained control over the basin-wide layout of subbasins. For example in Figure \@ref(fig:overview-catch-plot) (right) the number of subbasins per unit subcatchment area is fixed, resulting in more detail along the well-gaged Soda Butte Creek versus the rest of the Lamar basin. Here is that plot again with subcatchments coloured differently.

```{r splitter-bigger-plot, cache=TRUE, fig.align='center', fig.dim=c(6,8), fig.cap='QSWAT+ models for three Lamar River subcatchments with NWIS gages at the circled sites'}
...
```{r splitter-yellowstone-plot, cache=TRUE, fig.align='center', fig.dim=c(6, 9), fig.cap='Subcatchments in the Upper Yellowstone River basin, with Lamar River basin highlighted in red'}
...
The scheme is illustrated for the Yellowstone project by the next code chunk, which colors the 18 subcatchments from Figure \@ref(fig:splitter-yellowstone-plot) according to the stage at which they would be simulated. To effect a parallelized simulation, users could replace the `plot_catch` call below with a call to the SWAT+ simulator followed by an update to any dependent inlet files (*eg* using `rswat`).

```{r parallel-plot, cache=TRUE, fig.align='center', fig.dim=c(6, 9), fig.cap='Subcatchments colored according to their order in a SWAT+ parallelization scheme'}
